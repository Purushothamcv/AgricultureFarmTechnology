...
async def connect_to_mongodb():
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"✅ Users Database: {USERS_DATABASE_NAME}")
            logger.info(f"✅ Chatbot Database: {CHATBOT_DATABASE_NAME}")
            logger.info(f"✅ Legacy Database: {LEGACY_DATABASE_NAME} (backward compatibility)")
    ...
    # All retries failed
    logger.warning("="*60)
    logger.warning("⚠️  WARNING: Could not connect to MongoDB!")
...
async def create_user(name: str, email: str, password_hash: str) -> dict:
    ...
    try:
        ...
        logger.info(f"✅ User created successfully - email: {email} | user_id: {user_doc['user_id']}")
...
async def get_user_by_email(email: str) -> dict:
    ...
    try:
        ...
        if user:
            ...
            logger.debug(f"⚠️ User not found by email: {email}")
        return user
    except Exception as e:
...
async def get_user_by_id(user_id: str) -> dict:
    ...
    try:
        ...
        if user:
            ...
            logger.debug(f"⚠️ User not found by ID: {user_id}")
        return user
    except Exception as e:
...
async def update_user(user_id: str, update_data: dict) -> dict:
    ...
    try:
        ...
        if result:
            ...
            logger.warning(f"⚠️ User not found for update - user_id: {user_id}")
...
async def create_chat_session(user_id: str) -> dict:
    ...
    try:
        ...
        logger.info(f"✅ Chat session created - session_id: {session_doc['session_id']} | user_id: {user_id}")
...
async def get_chat_session(session_id: str) -> dict:
    ...
    try:
        ...
        if session:
            logger.debug(f"✅ Chat session found: {session_id}")
        else:
            logger.debug(f"⚠️ Chat session not found: {session_id}")
        return session
    except Exception as e:
...
async def save_message(session_id: str, role: str, content: str, metadata: dict = None) -> bool:
    ...
    try:
        ...
        if result.modified_count == 0:
            logger.warning(f"⚠️ Session not found for message insertion - session_id: {session_id}")
            return False
        
        logger.debug(f"✅ Message saved to session - session_id: {session_id} | role: {role}")
        return True
...
async def get_chat_history(session_id: str) -> dict:
    ...
    try:
        session = await chat_sessions_collection.find_one({"session_id": session_id})
        if session:
            logger.info(f"✅ Chat history fetched - session_id: {session_id} | messages: {len(session.get('messages', []))}")
        else:
...
async def get_user_sessions(user_id: str, limit: int = 20) -> list:
    ...
    try:
        ...
        logger.info(f"✅ User sessions fetched - user_id: {user_id} | count: {len(sessions)}")
...
async def delete_chat_session(session_id: str) -> bool:
    ...
    try:
        ...
        if result.deleted_count > 0:
            ...
            logger.warning(f"⚠️ Session not found for deletion - session_id: {session_id}")
            return False
